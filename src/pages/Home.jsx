                ...
                <div className="w-full max-w-3xl space-y-8">
                    ...
                    <div className="text-center space-y-5">
                        ...
                        <p className="max-w-xl mx-auto text-lg text-gray-400 leading-relaxed">
                            {appName} is ready. Configured with Vite, React, and
                            Tailwind CSS v4.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                        ...
                        <div
                            onClick={() =>
                                copy("bun run cook", setCopiedCommand)
                            }
                        ...
                        >
                            ...
                            <span
                                className={`transition-colors ${copiedCommand ? "text-emerald-400" : "text-gray-600 group-hover:text-gray-400"}`}
                            >
                                {copiedCommand ? <CheckIcon /> : <CopyIcon />}
                            </span>
                            <span
                                className={`absolute -top-8 left-1/2 -translate-x-1/2 bg-black/90 text-white text-xs px-2 py-1 rounded transition-opacity whitespace-nowrap pointer-events-none ${copiedCommand ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                            >
                                {copiedCommand
                                    ? "Copied!"
                                    : "Deploy to Salesive"}
                    ...
                    <p className="text-center text-sm text-gray-600">
                        Or push to{" "}
                        <code className="font-mono text-xs bg-white/6 px-1.5 py-0.5 rounded text-gray-400">
                            main
                        </code>{" "}
...
function Crumb({ children, highlight }) {
    return (
        <span
            className={`px-2 py-0.5 rounded text-[11px] font-medium ${highlight ? "bg-brand-primary/15 text-brand-primary" : "bg-white/8 text-gray-400"}`}
        >
            {children}
        </span>
    );
...
function CodeBlock({ children, onCopy, copied, className = "" }) {
    return (
        ...
        >
            ...
            <div
                className={`flex items-center gap-1.5 ml-4 flex-shrink-0 transition-colors ${copied ? "text-emerald-400" : "text-gray-600 group-hover:text-gray-400"}`}
            >
                {copied ? (
                    <>
                        <CheckIcon />
    ...
    return (
        ...
        >
            <div className="w-9 h-9 rounded-lg bg-white/8 border border-white/10 flex items-center justify-center flex-shrink-0 group-hover:border-brand-primary/25 transition-colors">
                <img
                    src={icon}
                    className={`w-5 h-5 ${imgClass}`}
                    alt={`${title} logo`}
                />
            </div>
            <div>
                <h3 className="text-sm font-semibold text-white group-hover:text-brand-primary transition-colors">
                    {title}
                </h3>
